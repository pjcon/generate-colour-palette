...
def generate_palette(n, jac_d=1, S=1, L=0.5, full_comparison=True, sl_freedom=True):
    ...
    def full_pairwise_score_SL_freedom(c):
        # Objective function for set of hues
        lh = len(c)//3
        sep = lambda i, a: a[i:i+lh]
        h, s, v = sep(0, c), sep(lh, c), sep(2*lh, c)
        
        H = combinations([HSLColor(i, j, k) for i,j,k in zip(h, s, v)], 2) # Generate every pair combination
        return np.sqrt(np.sum(np.square([1/cie_delta(h1, h2) for h1, h2 in H])))

    # Minimise the pairwise difference score between all colours.
    if full_comparison:
        if sl_freedom:
            objective_function = full_pairwise_score_SL_freedom
        else:
    ...
    if sl_freedom:
        initial_sats  = np.array([S]).repeat(n)
        initial_ligs  = np.array([L]).repeat(n)
        initial_hues = np.concatenate([initial_hues, initial_sats, initial_ligs])
